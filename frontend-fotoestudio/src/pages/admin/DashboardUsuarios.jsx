...
const DashboardUsuarios = () => {
    ...
    // --- 🎯 NUEVO ESTADO PARA FILTRAR POR ROL ---
    ...
    const obtenerUsuarios = () => {
        fetch(API_USUARIOS, { method: 'GET', headers: getHeaders() })
    ...
    const manejarEnvio = (e) => {
        ...
        // Solo incluimos la contraseña si es una inserción (Alta)
        ...
        fetch(url, {
            ...
            .then(async res => {
                if (res.ok) {
                    const usuarioModificadoOCreado = await res.json();

                    if (esEdicion) {
                        // CAMBIO: Actualizamos el estado local mapeándolo inmediatamente sin esperar al backend
                        setUsuarios(prevUsuarios =>
                            prevUsuarios.map(u => u.id === editandoId ? usuarioModificadoOCreado : u)
                        );
                        alert("✅ Cuenta de usuario actualizada en tiempo real");
                    } else {
                        // ALTA: Inyectamos el nuevo registro directo al fondo del arreglo visual
                        setUsuarios(prevUsuarios => [...prevUsuarios, usuarioModificadoOCreado]);
        ...
        if (window.confirm(`¿Seguro que deseas eliminar permanentemente al usuario: @${username}?`)) {
            fetch(`${API_USUARIOS}${id}/`, { method: 'DELETE', headers: getHeaders() })
                .then(res => {
                    if (res.ok) {
                        obtenerUsuarios();
                        alert("🗑️ Registro de usuario borrado de MySQL.");
                    }
                });
        }
    };
    ...
    // --- 🔍 FILTRO DINÁMICO COMBINADO (TEXTO + ROL SELECCIONADO) ---
    const usuariosFiltrados = usuarios.filter(u => {
        // 1. Validamos la coincidencia de texto
        const coincideTexto = u.username.toLowerCase().includes(buscar.toLowerCase()) ||
            u.email.toLowerCase().includes(buscar.toLowerCase()) ||
            `${u.first_name} ${u.last_name}`.toLowerCase().includes(buscar.toLowerCase());

        // 2. Validamos la coincidencia del rol del select
        const coincideRol = filtroRol === 'todos' || u.tipo_usuario.toLowerCase() === filtroRol.toLowerCase();
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-zinc-900 p-6 border border-zinc-800 rounded-3xl">
                <div>
                    ...
                    <p className="text-xs text-zinc-400 mt-1">Módulo ABCC para administración de cuentas, privilegios de staff y estados de actividad</p>
                </div>

                {/* Contenedor del buscador y el selector de rol */}
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    <input
                        type="text"
                        placeholder="Buscar por usuario, nombre..."
                        className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-600 w-full sm:w-64 font-medium"
                        value={buscar}
                        onChange={(e) => setBuscar(e.target.value)}
                    />

                    {/* 🔥 SELECTOR DINÁMICO DE FILTRO POR ROL 🔥 */}
                    <select
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                {/* TABLA DE CONTROL DE USUARIOS (Lado Derecho) */}
                    ...
                    <table className="w-full text-left border-collapse text-sm">
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {usuariosFiltrados.length > 0 ? (
                                    ...
                                    <tr key={u.id} className="hover:bg-zinc-800/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4 space-y-1.5">
                                            ...
                                            <div className="flex gap-2 text-[10px] font-bold">
                                                ...
                                                {u.is_staff && (
                                                    <span className="text-purple-400 bg-purple-500/5 px-1.5 py-0.2 rounded">
                                                        ⚙️ Staff
                                                    </span>
                                                )}
                                            </div>
                                ...
                                <tr>
                                    <td colSpan="3" className="text-center py-10 text-zinc-500 font-medium italic">No se encontraron cuentas con los filtros seleccionados.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
